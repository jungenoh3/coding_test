...
vector<int> check_row(int x, int y, const vector<vector<int>> &v){
...
vector<int> check_column(int x, int y, const vector<vector<int>> &v){
	vector<bool> num(10, true);
	vector<int> result;
...
vector<int> check_box(int x, int y, const vector<vector<int>> &v){
	vector<bool> num(10, true);
	vector<int> result;
...
void sdoku(int curr, vector<vector<int>> empty_pos, vector<vector<int>> v){
	...
	vector<int> row = check_row(x, y, v);
	if (row.empty()) return;
	vector<int> column = check_column(x, y, v);
	if (column.empty()) return;
	vector<int> box = check_box(x, y, v);
	if (box.empty()) return;

	vector<int> result = find_intersection(find_intersection(row, column), box);
